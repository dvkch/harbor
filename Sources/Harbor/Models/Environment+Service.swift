...
extension Environment {
    ...
    func inspect(service: any Serviceable) -> any Inspectable {
        ...
        case .heroku:
            let herokuService = service as! HerokuService
            switch herokuService {
            case .app(let app), .dyno(let app, _):
                let env = sshCodable(.command("heroku config --json -a \(app.name)"), type: [String: String].self)
                return ConcreteInspectable(inspectableImage: "", inspectableEnv: env.map { ($0, $1) })
            case .db:
                return ConcreteInspectable(inspectableImage: "", inspectableEnv: [])
            }
        }
    }
