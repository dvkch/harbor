...
extension Environment {
    ...
    static func generateServiceCompletion(_ input: String?, env: String?, filters: [Filter]) -> [String] {
        let environment = Config.shared.environments.first(where: { $0.alias == env })
        return environment?.services(filters: filters).map(\.serviceDisplayName).filter { $0.starts(with: input ?? "") } ?? []
    }
}
